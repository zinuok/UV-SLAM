...
queue<geometry_msgs::TransformStamped> gt_buf;
...
// without depth
std::vector<std::tuple<std::vector<sensor_msgs::ImuConstPtr>, sensor_msgs::PointCloudConstPtr, sensor_msgs::ImageConstPtr, geometry_msgs::TransformStamped>>
getMeasurementsGT()
{
    std::vector<std::tuple<std::vector<sensor_msgs::ImuConstPtr>, sensor_msgs::PointCloudConstPtr, sensor_msgs::ImageConstPtr, geometry_msgs::TransformStamped>> measurements;

    while (true)
    {

        if (imu_buf.empty() || feature_buf.empty() || gt_buf.empty() || img_buf.empty())
        {
//            cout << "empty! " << gt_buf.size() << endl;
            return measurements;
        }
...
//            cout << "throw image! " << endl;
        ...
        geometry_msgs::TransformStamped lastGT_msg;
        geometry_msgs::TransformStamped latestGT_msg;



        while (gt_buf.front().header.stamp.toSec() < img_msg->header.stamp.toSec())
        {
            lastGT_msg = gt_buf.front();
            gt_buf.pop();
            if(gt_buf.empty())
                break;
        }
        ...
        if(gt_buf.front().header.stamp.toSec() - img_msg->header.stamp.toSec() >
                img_msg->header.stamp.toSec() - lastGT_msg.header.stamp.toSec())
            latestGT_msg = lastGT_msg;
        else
            latestGT_msg = gt_buf.front();


//        cout << "gt_t: " << std::fixed << latestGT_msg.header.stamp.toSec() << endl;


        measurements.emplace_back(make_tuple(IMUs, img_msg, latestImg_msg, latestGT_msg));
    }
...
void process()
{
    ...
    //            foutC.close();
                ...
                pubDepthCompletion(estimator, header);
        ...
        else
        {
            ...
            con.wait(lk, [&]
                     {
                return (measurements = getMeasurementsGT()).size() != 0;
                     });
            lk.unlock();
    ...
    //            cout << "\n" << endl;
                ...
                estimator.processImage(image, image_line, img_msg->header, latest_img_, get<3>(measurement));
...
//    pub_only_no_loop_pose = n.advertise<geometry_msgs::PoseArray>("/vins_estimator/no_loop_pose", 1000);
    ...
    if (USE_EUROC)
    {
        ifstream inFileGT(GT_POSE_PATH);
        string line;
        string time, t_x, t_y, t_z, q_w, q_x, q_y, q_z, v_x, v_y, v_z,
                b_w_x, b_w_y, b_w_z, b_a_x, b_a_y, b_a_z;

        cout << "open GT file: " << GT_POSE_PATH << endl;

        while (getline(inFileGT, line))
        {
            ...
            getline(ss, b_a_y, ',');
            getline(ss, b_a_z, ',');
            geometry_msgs::TransformStamped gt_msg;
            std_msgs::Header header;
            header.stamp.fromNSec(stod(time));
            header.frame_id = "world";
            gt_msg.header = header;
            gt_msg.transform.translation.x = stod(t_x);
            gt_msg.transform.translation.y = stod(t_y);
            gt_msg.transform.translation.z = stod(t_z);
            gt_msg.transform.rotation.w = stod(q_w);
            gt_msg.transform.rotation.x = stod(q_x);
            gt_msg.transform.rotation.y = stod(q_y);
            gt_msg.transform.rotation.z = stod(q_z);
            gt_buf.push(gt_msg);
        }
        cout << gt_buf.size() << endl;
    }
